...
public:
		...
		StereoCalib(const vector<string>& imagelist, Size boardSize, float squareSize, bool displayCorners = false, bool useCalibrated = true, bool showRectified = true)
	{
		...
		//namedWindow("WindowThing", WINDOW_AUTOSIZE);
		...
		FileStorage fs(INTRINSICS_FILE_PATH, FileStorage::WRITE);
		if (fs.isOpened())
		{
		...
		fs.open(EXTRINSICS_FILE_PATH, FileStorage::WRITE);
		if (fs.isOpened())
		{
	...
	int init_calibration(int argc, char** argv)
	{
		Size boardSize;
		string imagelistfn;
		bool showRectified;
		...
		cout << "width" << boardSize.width << endl;
		boardSize.height = parser.get<int>("h");
		cout << "height" << boardSize.height << endl;
		float squareSize = parser.get<float>("s");
		cout << "size" << squareSize << endl;
		/*if (!parser.check())
		{
			parser.printErrors();
			return 1;
